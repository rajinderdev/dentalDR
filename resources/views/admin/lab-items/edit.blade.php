...
<div class="bg-white rounded-lg shadow-sm p-4 md:p-6">
    ...
    <h1 class="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6">Edit Lab Item</h1>

    <form id="editLabItemForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="border border-gray-200 rounded-lg p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div>
                    ...
                    <select id="ComponentCategoryID" name="ComponentCategoryID"
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ $item->ComponentCategoryID == $category->id ? 'selected' : '' }}>{{ $category->ItemTitle }}</option>
                        @endforeach
                    </select>
                </div>
                {{-- Component Name --}}
                <div>
                    ...
                    <input type="text" id="ComponentName" name="ComponentName" value="{{ $item->ComponentName }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mt-4">
                ...
                <div>
                    ...
                    <input type="text" id="ComponentDescription" name="ComponentDescription" value="{{ $item->ComponentDescription }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                ...
                <div>
                    ...
                    <input type="number" id="LabWorkCost" name="LabWorkCost" step="0.01" min="0" value="{{ $item->LabWorkCost ?? 0 }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                           placeholder="0.00">
                </div>
            </div>
...
$('#editLabItemForm').on('submit', function(e) {
    e.preventDefault();
    if ($(this).valid()) {
        var formData = new FormData(this);
        $.ajax({
            url: "{{ route('admin.lab-items.update', $item->LabWorkComponentID) }}",
            type: "POST",
            data: formData,
            contentType: false,
            cache: false,
            ...
            error: function(xhr) {
                let errorMessage = 'An error occurred while updating lab item.';
                if (xhr.responseJSON) {
