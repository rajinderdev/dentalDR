@extends('schoolManagement.layout.master')
    ...
    <div class="modal-dialog">
        <div class="modal-content border-0 shadow-lg">
            ...
            <div class="modal-footer justify-content-center">
                ...
                <a href="{{ route('schoolManagement.skills.index') }}" role="button" class="btn btn-danger">Ok</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(function () {
        skills_table = $('.skills-datatable').DataTable({
            ...
            select: true,
            ajax: {
            url: "{{ route('schoolManagement.skills.index') }}",
            },
            columns: [
                {
                    name: 'id',
...
// Add skills 
    ...
    $('#addSkills').on('submit', function(e) {
        ...
        if ($(this).valid()) {
            $.ajax({
                url: "{{ route('schoolManagement.skills.store') }}",
                type: "POST",
    ...
    function editSkill(id){
        ...
        $.ajax({
            url: "{{url('school-admin/skills/edit')}}"+"/"+id,
            type: "get",
            success: function (res) {
                if(res.success==true){
                    $("#id").val(id);
    ...
    $('#editSkills').on('submit', function(e) {
        ...
        if ($(this).valid()) {
            $.ajax({
                url: "{{ route('schoolManagement.skills.update') }}",
                type: "POST",
                data: new FormData(this),
                contentType: false,
                cache: false,
 ...
 $('#deleteSkill').on('submit', function(e) {
    ...
    if ($(this).valid()) {
        $.ajax({
            url: "{{  route('schoolManagement.skills.delete') }}",
            type: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
