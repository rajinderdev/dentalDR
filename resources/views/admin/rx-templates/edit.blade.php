...
<div class="bg-white rounded-lg shadow-sm p-4 md:p-6">
    ...
    <h1 class="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6">Edit Rx-Template</h1>

    <form id="editRxTemplateForm" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-3 md:space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" id="PrescriptionTemplateName" name="PrescriptionTemplateName" value="{{ $template->PrescriptionTemplateName }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <textarea id="PrescriptionTemplateDesc" name="PrescriptionTemplateDesc" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                                  placeholder="Enter template description">{{ $template->PrescriptionTemplateDesc }}</textarea>
                    </div>
                    {{-- Prescription Note --}}
                    <div>
                        ...
                        <textarea id="PrescriptionNote" name="PrescriptionNote" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                                  placeholder="Enter prescription note">{{ $template->PrescriptionNote }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="border border-gray-200 rounded-lg p-4 md:p-6">
            ...
            <div id="no-medicines-msg" class="text-center py-4 text-gray-400 text-sm" style="display: none;">
...
var existingMedicines = @json($templateMedicines);
...
function addMedicineRow(data) {
    data = data || {};
    var escapedMedicineName = $('<div>').text(data.MedicineName || '').html();
    var escapedFrequency = $('<div>').text(data.Frequency || '').html();
    var escapedDosage = $('<div>').text(data.Dosage || '').html();
    var escapedDuration = $('<div>').text(data.Duration || '').html();
    var escapedDrugNote = $('<div>').text(data.DrugNote || '').html();
    ...
    var row = `
        <tr class="medicine-row border-b border-gray-100" data-index="${medicineRowIndex}">
            ...
            <td class="px-3 py-2">
                ...
                <input type="text" name="template_medicines[${medicineRowIndex}][MedicineName]" value="${escapedMedicineName}"
            ...
            <td class="px-3 py-2">
                <input type="text" name="template_medicines[${medicineRowIndex}][Frequency]" value="${escapedFrequency}"
                       class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                       placeholder="e.g., Twice daily">
            </td>
            <td class="px-3 py-2">
                <input type="text" name="template_medicines[${medicineRowIndex}][Dosage]" value="${escapedDosage}"
                       class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                       placeholder="e.g., 500mg">
            </td>
            <td class="px-3 py-2">
                <input type="text" name="template_medicines[${medicineRowIndex}][Duration]" value="${escapedDuration}"
                       class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                       placeholder="e.g., 5 days">
            </td>
            <td class="px-3 py-2">
                <input type="text" name="template_medicines[${medicineRowIndex}][DrugNote]" value="${escapedDrugNote}"
                       class="w-full px-2 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                       placeholder="Note">
            </td>
...
$('#editRxTemplateForm').on('submit', function(e) {
    e.preventDefault();
    if ($(this).valid()) {
        var formData = new FormData(this);
        $.ajax({
            url: "{{ route('admin.rx-templates.update', $template->PrescriptionTemplateMasterID) }}",
            type: "POST",
            data: formData,
            contentType: false,
            cache: false,
            ...
            error: function(xhr) {
                let errorMessage = 'An error occurred while updating Rx-Template.';
                if (xhr.responseJSON) {
