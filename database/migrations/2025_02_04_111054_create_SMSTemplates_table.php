<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('SMSTemplates', function (Blueprint $table) {
            $table->uuid('SMSTemplateID')->default('newid()');
            $table->uuid('ClinicID')->nullable();
            $table->uuid('SituationID')->nullable();
            $table->unsignedInteger('SMSCategoryID')->nullable();
            $table->string('FromPhoneNumber', 50)->nullable();
            $table->string('FromSenderID', 50)->nullable();
            $table->text('Message')->nullable();
            $table->dateTime('EffectiveDate')->nullable()->useCurrent();
            $table->boolean('IsDeleted')->nullable()->default(false);
            $table->dateTime('CreatedOn')->nullable()->useCurrent();
            $table->string('CreatedBy', 50)->nullable();
            $table->dateTime('LastUpdatedOn')->nullable()->useCurrent();
            $table->string('LastUpdatedBy', 50)->nullable();

            $table->primary(['SMSTemplateID'], 'pk_smstemplates_1');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('SMSTemplates');
    }
};
