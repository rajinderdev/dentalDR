...
<div class="bg-white rounded-lg shadow-sm p-4 md:p-6">
    ...
    <h1 class="text-xl md:text-2xl font-bold text-gray-800 mb-4 md:mb-6">Edit Chair</h1>

    <form id="editChairForm" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="border border-gray-200 rounded-lg p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="Title" name="Title" value="{{ $chair->Title }}"
                ...
                <div>
                    ...
                    <input type="text" id="Description" name="Description" value="{{ $chair->Description }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
        ...
        <div class="border border-gray-200 rounded-lg p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                <div>
                    ...
                    <input type="time" id="start_time" name="start_time" value="{{ $chairSlot && $chairSlot->StartDatetime ? $chairSlot->StartDatetime->format('H:i') : '08:00' }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
                </div>
                <div>
                    <label for="end_time" class="block text-sm font-medium text-gray-700 mb-2">End Time</label>
                    <input type="time" id="end_time" name="end_time" value="{{ $chairSlot && $chairSlot->EndDateTime ? $chairSlot->EndDateTime->format('H:i') : '21:30' }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
                </div>
                <div>
                    <label for="slot_interval" class="block text-sm font-medium text-gray-700 mb-2">Time Interval (in minutes)</label>
                    <input type="number" id="slot_interval" name="slot_interval" min="1" max="480" value="{{ $chairSlot->SlotInterval ?? 15 }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                           placeholder="15">
                </div>
            </div>
...
$('#editChairForm').on('submit', function(e) {
    e.preventDefault();
    if ($(this).valid()) {
        var formData = new FormData(this);
        $.ajax({
            url: "{{ route('admin.chairs.update', $chair->ChairID) }}",
            type: "POST",
            data: formData,
            contentType: false,
            cache: false,
            ...
            error: function(xhr) {
                let errorMessage = 'An error occurred while updating chair.';
                if (xhr.responseJSON) {
