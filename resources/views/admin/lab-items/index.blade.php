...
<div class="bg-white rounded-lg shadow-sm p-6">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200" id="lab-items-table">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Component Category</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Component Name</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Description</th>
                    <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Lab Work Cost</th>
...
$(document).ready(function() {
    var table = $('#lab-items-table').DataTable({
        processing: true,
        serverSide: true,
        searching: false,
        ajax: {
            url: "{{ route('admin.lab-items.index') }}",
            data: function(d) {
        ...
        language: {
            ...
            zeroRecords: 'No lab items found',
    ...
    $('#categoryFilter').on('change', function() {
        ...
        var url = "{{ route('admin.lab-items.create') }}";
        if (category) {
    ...
    $('#deleteSelectedBtn').on('click', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.lab-items.bulk-delete") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
...
function deleteLabItem(itemId) {
    Swal.fire({
        title: 'Are you sure?',
        text: "This lab item will be removed from the system!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("admin.lab-items.destroy", ":itemId") }}'.replace(':itemId', itemId),
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function(response) {
                    if (response.success) {
                        Swal.fire('Deleted!', 'Lab item has been deleted.', 'success').then(() => {
